import pyaudio
import wave
import threading
import queue
import speech_recognition as sr
import numpy as np
import simpleaudio as sa
import webrtcvad
import subprocess
import os
import random
import tkinter as tk
from tkinter import ttk
import time
import traceback

# Make root and status_label global
root = None
status_label = None

def tinker():
    global root, status_label
    root = tk.Tk()
    root.title("Screen Recorder")
    root.geometry("300x100")
    root.attributes('-topmost', True)  

    status_label = ttk.Label(root, text="Waiting for sound...", font=("Arial", 12))
    status_label.pack(pady=20)

    root.mainloop()

def get_random_file_recording():
    intro_files_recording = []
    if os.path.exists('All_Intro_Recording'):
        intro_files_recording = [f for f in os.listdir('All_Intro_Recording') if os.path.isfile(os.path.join('All_Intro_Recording', f))]

    if intro_files_recording:
        random_file = random.choice(intro_files_recording)
        print(f"Selected file: {random_file}")
        return random_file
    else:
        print("No files found in All_Intro_Recording directory")
        return None

def get_random_file_not_understand():
    intro_files_understand = []
    if os.path.exists('All_Understands_Recording'):
        intro_files_understand = [f for f in os.listdir('All_Understands_Recording') if os.path.isfile(os.path.join('All_Understands_Recording', f))]

    if intro_files_understand:
        random_file_not_understand = random.choice(intro_files_understand)
        print(f"Selected file: {random_file_not_understand}")
        return random_file_not_understand
    else:
        print("No files found in All_Understands_Recording directory")
        return None

# translator = googletrans.Translator()

from deep_translator import GoogleTranslator

r = sr.Recognizer()

audio_queue = queue.Queue()

resume_event = threading.Event()
resume_event.set()  

chunk = 1024  
sample_format = pyaudio.paInt16  
channels = 1
print(channels)
fs = 44100  
seconds = 5
filename = "output.wav"

audio_playing = threading.Event()

audio_lock = threading.Lock()
audio_paused = threading.Event()

def pause_audio_processing():
    """Pause audio processing and clear the audio queue."""
    global audio_paused, resume_event
    
    with audio_lock:
        if not audio_paused.is_set():
            print("Pausing audio processing...")
            # First clear the flag to prevent new recordings
            resume_event.clear()
            
            # Wait for any ongoing playback to complete
            if audio_playing.is_set():
                print("Waiting for ongoing audio playback to complete...")
                start_time = time.time()
                while audio_playing.is_set() and time.time() - start_time < 5:
                    time.sleep(0.1)
                    
            # Clear the queue
            clear_audio_queue()
            
            # Mark as paused
            audio_paused.set()
            
            if status_label:
                status_label.config(text="Recording paused")
            
            # Add small delay to ensure pause takes effect
            time.sleep(0.5)
            
            print("Audio processing successfully paused.")

def resume_audio_processing():
    """Resume audio processing after being paused."""
    global audio_paused, resume_event
    
    with audio_lock:
        # Always set resume_event regardless of audio_paused state
        resume_event.set()
        
        if audio_paused.is_set():
            print("Resuming audio processing...")
            
            # Ensure any ongoing audio playback from visualization is stopped
            try:
                import pygame
                if pygame.mixer.get_init() and pygame.mixer.music.get_busy():
                    pygame.mixer.music.fadeout(500)  # Use fadeout instead of stop
                    time.sleep(0.6)  # Wait for fadeout to complete
            except (ImportError, AttributeError):
                pass  # pygame might not be available or initialized
            
            # Reset to clear state
            audio_paused.clear()
            
            if status_label:
                status_label.config(text="Waiting for sound...")
            
            # Add small delay to ensure resume takes effect
            time.sleep(1.0)  # Increased delay to ensure clean transition
            
            print("Audio processing successfully resumed.")

def clear_audio_queue():
    """Mengosongkan semua item dalam antrian audio."""
    cleared = 0
    while not audio_queue.empty():
        try:
            audio_queue.get_nowait()
            audio_queue.task_done()
            cleared += 1
        except queue.Empty:
            break
    print(f"Antrian audio dibersihkan, {cleared} item dihapus.")

def detect_sound(data):
    """Deteksi apakah ada suara dalam data audio."""
    audio_data = np.frombuffer(data, dtype=np.int16)
    # Increased threshold for better noise rejection
    return np.max(np.abs(audio_data)) > 5000

def play_audio(audio_file):
    """Putar audio dalam thread terpisah."""
    print(audio_file)
    if not audio_playing.is_set():  
        audio_playing.set()  
        try:
            full_path = None
            possible_paths = [
                os.path.join('All_Intro_Recording', audio_file),
                os.path.join('All_Understands_Recording', audio_file)
            ]
            
            for path in possible_paths:
                if os.path.exists(path):
                    full_path = path
                    break
                    
            if full_path is None:
                print(f"File audio tidak ditemukan di lokasi yang diharapkan: {audio_file}")
                return
                
            subprocess.run(
                ["ffplay", "-nodisp", "-autoexit", "-sync", "ext", full_path],
                check=True,
                stdout=subprocess.DEVNULL,
                stderr=subprocess.DEVNULL
            )
        except FileNotFoundError:
            print("ffplay tidak ditemukan. Pastikan ffmpeg terinstall")
        except Exception as e:
            print(f"Error saat memutar audio: {e}")
        finally:
            audio_playing.clear()  

def record_audio():
    if status_label:
        status_label.config(text="Waiting for sound...")
    print("Menunggu suara untuk memulai perekaman...")
    """Fungsi untuk merekam audio dan memasukkannya ke dalam antrian."""
    p = pyaudio.PyAudio()  

    stream = p.open(format=sample_format,
                    channels=channels,
                    rate=fs,
                    frames_per_buffer=chunk,
                    input=True)

    frames = []  

    try:
        while True:
            resume_event.wait()  
            data = stream.read(chunk)

            if detect_sound(data):
                print("Suara terdeteksi, mulai merekam...")
                if status_label:
                    status_label.config(text="Recording in progress...")
                random_file = get_random_file_recording()
                if random_file:
                    play_thread = threading.Thread(target=play_audio, args=(random_file,))
                else:
                    print("No intro file found to play")
                    play_thread = None
                play_thread.start()
                frames.append(data)

                chunks_recorded = 0
                silence_chunks = 0
                min_chunks = int(fs / chunk * seconds)  
                
                while True:
                    data = stream.read(chunk)
                    frames.append(data)
                    chunks_recorded += 1

                    if chunks_recorded >= min_chunks:
                        if detect_sound(data):
                            silence_chunks = 0  
                        else:
                            silence_chunks += 1
                            
                        if silence_chunks >= int(fs / chunk):
                            break
                    
                audio_queue.put(b''.join(frames))
                frames = []
                print("Perekaman selesai, menunggu resume untuk melanjutkan...")
                if status_label:
                    status_label.config(text="Recording complete")
                
                resume_event.clear()  
    except Exception as e:
        print(f"Terjadi kesalahan selama perekaman: {e}")
    finally:
        stream.stop_stream()
        stream.close()
        p.terminate()

def process_audio():
    """Function to process audio from the queue and perform transcription."""
    while True:
        try:
            # Check if processing is paused before trying to get from queue
            if audio_paused.is_set():
                time.sleep(0.2)  # Short sleep when paused to reduce CPU usage
                continue
                
            # Wait for audio data with a timeout to allow checking for pause state
            try:
                audio_data = audio_queue.get(timeout=0.5)
            except queue.Empty:
                continue
                
            if audio_data is None:
                break
                
            # Check again if we've been paused before processing
            if audio_paused.is_set():
                # Put the audio data back in the queue if we're paused
                audio_queue.put(audio_data)
                time.sleep(0.2)
                continue
                
            try:
                # Create a new audio file with a timestamp to prevent conflicts
                timestamp = int(time.time())
                temp_filename = f"output_{timestamp}.wav"
                
                wf = wave.open(temp_filename, 'wb')
                wf.setnchannels(channels)
                wf.setsampwidth(pyaudio.PyAudio().get_sample_size(sample_format))
                wf.setframerate(fs)
                wf.writeframes(audio_data)
                wf.close()

                with sr.AudioFile(temp_filename) as source:
                    audio = r.record(source)
                    try:
                        transcription = r.recognize_google(audio, language='id-ID')
                        translate = GoogleTranslator(source='auto', target='en').translate(transcription)
                        print(f"Transcription: {translate}")
                        
                        # Return translation to be used by main.py
                        yield translate
                    except sr.UnknownValueError:
                        random_file_not_understand = get_random_file_not_understand()
                        if random_file_not_understand:
                            play_thread = threading.Thread(target=play_audio, args=(random_file_not_understand,))
                            play_thread.start()
                        else:
                            print("No intro file found to play")
                        print("Google Speech Recognition could not understand audio.")
                        # Always resume after failed recognition
                        resume_audio_processing()
                    except sr.RequestError as e:
                        print(f"Google Speech Recognition request failed; {e}")
                
                # Clean up temporary file
                try:
                    os.remove(temp_filename)
                except:
                    pass
                    
            except Exception as e:
                print(f"Unexpected error occurred: {e}")
                traceback.print_exc()
            finally:
                audio_queue.task_done()
        except Exception as outer_e:
            print(f"Critical error in audio processing loop: {outer_e}")
            traceback.print_exc()
            time.sleep(1)  # Prevent tight loop in case of repeated errors

# Start GUI thread first
gui_thread = threading.Thread(target=tinker, daemon=True)
gui_thread.start()

# Give GUI time to initialize
time.sleep(1)

record_thread = threading.Thread(target=record_audio, daemon=True)
record_thread.start()

process_thread = threading.Thread(target=process_audio, daemon=True)
process_thread.start()