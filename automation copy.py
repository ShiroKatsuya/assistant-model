import os
import subprocess
import difflib

def find_best_app_match(query, apps):
    # Normalize the query and names to lower-case for a fair comparison.
    query_lower = query.lower()
    app_names_lower = [app["name"].lower() for app in apps]
    
    # First use difflib with a lower cutoff to find a close match.
    matches = difflib.get_close_matches(query_lower, app_names_lower, n=1, cutoff=0.4)
    if matches:
        # Return the app with the matching lower-case name.
        return next(app for app in apps if app["name"].lower() == matches[0])
    
    # Fallback: check if the query substring is in any app name.
    for app in apps:
        if query_lower in app["name"].lower():
            return app
    return None

app_name_input = input("Enter application name to open: ")

try:
    check_apps = subprocess.check_output(["powershell", "Get-StartApps"], text=True)

    apps = []
    for line in check_apps.splitlines():
        line = line.strip()
        # Skip empty lines or header rows if present.
        if not line or "AppUserModelId" in line:
            continue

        parts = line.rsplit(None, 1)
        if len(parts) != 2:
            continue
        display_name, app_id = parts[0], parts[1]
        apps.append({"name": display_name, "id": app_id})

    if not apps:
        print("No apps found.")
        exit(1)

    # Use the improved matching logic.
    matched_app = find_best_app_match(app_name_input, apps)

    if matched_app:
        corrected_name = matched_app["name"]
        if corrected_name.lower() != app_name_input.lower():
            print(f"Correcting '{app_name_input}' to '{corrected_name}'.")
        else:
            print(f"Found app '{corrected_name}'.")
        try:
            subprocess.run(["powershell", f"Start-Process shell:AppsFolder\\{matched_app['id']}"], check=True)
            print(f"Launched {corrected_name}")
        except subprocess.CalledProcessError as e:
            print(f"Error launching app: {e}")
    else:
        print(f"Could not find an app matching '{app_name_input}'.")

except subprocess.CalledProcessError as e:
    print(f"Error running Get-StartApps: {e}")