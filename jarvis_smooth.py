...
def update_animation(status):
    ...
    # Use a continuous time-based phase for smoother animations
    phase_param = time.time()

    # Map all possible status messages to three main animation states
    if status in ["Waiting for sound...", "Waiting to resume...", "Resume for sound...", "Pause for sound..."]:
        # Sound wave animation for all waiting states
        draw_sound_wave(animation_canvas, width, height, phase=0) 
        if progress_bar:
    ...
    elif status in ["Recording in progress...", "Mendeteksi suara, mulai merekam..."]:
        # Recording animation for active recording states
        recording_phase = (time.time() * 0.1) % 1
        draw_recording_animation(animation_canvas, width, height, phase=recording_phase)
        if progress_bar:
            # Update progress bar based on the visual phase
            progress_bar.config(value=recording_phase * 100)

    elif status in ["Recording complete", "Perekaman selesai, menunggu pemrosesan..."]:
        # Complete animation for completion and processing states
    ...
    # Continue animation loop - maintain ~30fps for smooth animation
    animation_canvas.after(33, lambda: update_animation(status))
...
def setup_jarvis_ui(root_window, pause_func, resume_func):
    ...
    # Start animation loop with initial "Waiting for sound..." state
    animation_canvas.after(100, lambda: update_animation("Waiting for sound..."))
...
def update_status(status_text):
    ...
    if status_label:
        ...
        if status_label.cget("text") != status_text:
            # Update the status label with the new text
            status_label.config(text=status_text)
            
            # Immediately update the animation to match the new status
            # This ensures animations start immediately when status changes
            animation_canvas.after_cancel(animation_canvas.after_idle(lambda: None))  # Cancel any pending animation updates
