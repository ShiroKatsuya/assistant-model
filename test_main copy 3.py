...
def run_with_inputs(input_text):
    ...
    try:
        if visualization_running:
            ...
            max_start_wait = 60  # Increased timeout for audio generation
            ...
            max_playback_wait = 300  # Extended to 5 minutes to handle longer audio files
            ...
            if pygame.mixer.get_init() and pygame.mixer.music.get_busy():
                ...
                pygame.mixer.music.fadeout(1000)  # Fade out over 1 second instead of abrupt stop
            ...
            if audio_thread and audio_thread.is_alive():
                ...
                audio_thread.join(timeout=15)  # Increased timeout for thread completion
                
            # Process any remaining events to keep the visualization responsive
            for _ in range(5):  # Process a few event cycles
                plt.pause(0.1)
                if plt.fignum_exists(plt.gcf().number):
                    plt.gcf().canvas.flush_events()
        ...
        # Quietly prepare for next input without unnecessary log messages
