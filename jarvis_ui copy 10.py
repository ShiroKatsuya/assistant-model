...
TEXT_COLOR = "#E0E0E0"
# FONT_FAMILY = "Consolas"  # Monospaced font with smooth curves # Commented out original

# Define some futuristic colors (complementing existing ones)
NEON_BLUE = "#00FFFF"  # Bright Cyan
NEON_ORANGE = "#FF7F00" # Brighter Orange
NEON_CYAN = "#00FFFF" # Added definition for Bright Cyan
DEEP_SPACE_BG = "#0A0A1A" # Very dark blue/black
METAL_BG = "#333340" # Dark metallic blue/grey
METAL_HIGHLIGHT = "#555566" # Lighter metallic tone
METAL_SHADOW = "#1A1A2B" # Darker shadow for metal
FUTURISTIC_FONT = "Orbitron" # Try this font, fallback needed
NEON_GREEN = "#39FF14" # Added for completion/success states
NEON_RED = "#FF003C" # Added for recording/active states

# Attempt to find the futuristic font, otherwise fallback
try:
    # Check if font exists (this is a basic check)
    import tkinter.font
    fonts = list(tkinter.font.families())
    if FUTURISTIC_FONT not in fonts:
        FONT_FAMILY = "Segoe UI" # Fallback font
    else:
        FONT_FAMILY = FUTURISTIC_FONT
except:
    FONT_FAMILY = "Segoe UI" # Default fallback

# UI component references
animation_canvas = None
progress_bar = None
notification_label = None
notification_frame = None # Added reference for notification frame
status_label = None
root = None
...
def update_animation(status):
    ...
    global animation_canvas, progress_bar, notification_label, root
    ...
    animation_canvas.after(20, lambda: update_animation(status)) # Aim for ~30fps

def show_notification(message):
    """Show a temporary notification with advanced 3D futuristic styling"""
    global notification_frame, notification_label, root
    
    if notification_frame is None:
        # Create the multi-layered notification panel
        
        # Outermost neon glow frame
        outer_glow_frame = tk.Frame(
            root, 
            bg=NEON_BLUE,
            bd=0
        )
        ...
        # Metallic border layer with raised effect
        ...
        # Main notification panel with inset appearance
        ...
        notification_frame.pack(padx=1, pady=1)
        
        # Inner content panel with dark background
        ...
        inner_panel.pack(padx=3, pady=3, fill="both", expand=True)
        
        # Notification icon/decorative element
        icon_frame = tk.Frame(inner_panel, bg=DEEP_SPACE_BG)
        icon_frame.pack(side="left", padx=5)
        
        # Create circular icon with neon border
        icon_size = 15
        ...
        # Draw concentric circles for tech appearance
        icon_canvas.create_oval(
            1, 1, icon_size-1, icon_size-1,
            outline=NEON_BLUE, fill=DEEP_SPACE_BG
        )
        icon_canvas.create_oval(
            4, 4, icon_size-4, icon_size-4,
            outline=NEON_CYAN, fill=DEEP_SPACE_BG
        )
        icon_canvas.create_oval(
            7, 7, icon_size-7, icon_size-7,
            outline=NEON_BLUE, fill=NEON_BLUE
        )
        
        # Text container with metallic edge
        text_container = tk.Frame(
            inner_panel, 
            bg=DEEP_SPACE_BG
        )
        text_container.pack(side="left", padx=5, fill="both", expand=True)
        ...
        # Add a neon decorative line beneath text
        separator = tk.Frame(
            text_container, 
            height=1, 
            bg=NEON_BLUE
    ...
    # Update text when notification is reused
    notification_label.config(text=message)
    
    # Position at top center with animation effect
    notification_frame.place_forget() # Hide first to reset any previous placement
    notification_frame.place(relx=0.5, y=-50, anchor="n") # Start above visible area
    
    # Animate entrance
    def animate_in(y_pos):
        if y_pos <= 20:
            notification_frame.place(relx=0.5, y=y_pos, anchor="n")
            # Continue animation with easing effect (slows down as it approaches target)
            next_y = y_pos + max(1, int((20 - y_pos) / 2))
            root.after(10, lambda: animate_in(next_y))
    
    # Start animation
    animate_in(-40)
    
    # Schedule fade-out
    def animate_out(y_pos=20, opacity=1.0):
        if y_pos < -50:  # Animation complete
        ...
        # Continue animation
        root.after(15, lambda: animate_out(y_pos-3, opacity-0.05))
    
    # Schedule disappearance with animation
    root.after(3000, lambda: animate_out())

def create_button(parent, text, command, is_primary=True):
    """Create a futuristic 3D button with advanced holographic effects"""
    
    # Colors based on button type
    neon_color = NEON_ORANGE if is_primary else NEON_BLUE
    accent_color = "#FF9500" if is_primary else "#00DDFF"  # Brighter variant for highlights
    hover_color = "#FFFFFF"  # Bright white on hover
    base_bg = METAL_BG
    highlight_bg = METAL_HIGHLIGHT
    shadow_color = METAL_SHADOW
    
    # Multi-layered frame structure for advanced 3D effect
    
    # Outermost shadow layer for depth
    shadow_frame = tk.Frame(
        parent,
        bg=shadow_color,
        bd=0
    ...
    # Outer glow frame that changes color on hover
    glow_frame = tk.Frame(
        shadow_frame, 
        bg=shadow_color,  # Start with no glow
        bd=0
    )
    glow_frame.pack(padx=2, pady=2)
    
    # Metallic outer beveled edge
    bevel_frame = tk.Frame(
        glow_frame, 
        bg=base_bg,
        bd=2, 
        relief="raised"  # Raised outer edge
    )
    bevel_frame.pack()
    
    # Middle frame for creating inset appearance
    mid_frame = tk.Frame(
        bevel_frame, 
        bg=base_bg,
        bd=1, 
        relief="groove"  # Groove adds a more technical look
    )
    mid_frame.pack(padx=1, pady=1)
    
    # Inner frame with neon trim
    inner_frame = tk.Frame(
        mid_frame,
        bg=base_bg,
        bd=1,
        highlightbackground=neon_color,
        highlightthickness=0  # Hidden initially, shows on hover
    )
    ...
    # Add a subtle metallic gradient effect to button surface
    surface_canvas = tk.Canvas(
        button_frame,
        bg=base_bg,
        highlightthickness=0,
        bd=0,
    ...
    surface_canvas.pack(fill="x")
    
    # Create metallic gradient effect on canvas
    def draw_metallic_surface():
        width = surface_canvas.winfo_width()
        height = surface_canvas.winfo_height()
        if width <= 1:  # Not yet properly sized
            surface_canvas.after(50, draw_metallic_surface)
            return
        ...
        for y in range(height):
            ...
            ratio = y / height
            if ratio < 0.5:  # Top half - lighter
                intensity = int(70 + (1 - ratio*2) * 40)
            else:  # Bottom half - darker
                intensity = int(70 - (ratio-0.5) * 2 * 30)
                
            color = f"#{intensity:02x}{intensity:02x}{intensity+10:02x}"
            surface_canvas.create_line(0, y, width, y, fill=color)
            
        # Add horizontal highlight lines for tech effect
        surface_canvas.create_line(0, 1, width, 1, fill=highlight_bg, width=1)
        surface_canvas.create_line(0, height-1, width, height-1, fill=shadow_color, width=1)
        
        # Add text with shadow effect for depth
        text_y = height // 2
        # Shadow first
        surface_canvas.create_text(width//2+1, text_y+1, text=text, fill=shadow_color, font=(FONT_FAMILY, 10, "bold"))
        # Main text
        surface_canvas.create_text(width//2, text_y, text=text, fill=neon_color, font=(FONT_FAMILY, 10, "bold"), tags="text")
    
    # Schedule initial drawing after widget is mapped
    surface_canvas.after(50, draw_metallic_surface)
    
    # Track button state
    button_state = {"active": False, "pressed": False}
    ...
    def on_enter(e):
        ...
        button_state["active"] = True
        
        # Activate neon glow
        glow_frame.config(bg=neon_color)
        inner_frame.config(highlightthickness=1)
        
        # Update surface appearance
        width = surface_canvas.winfo_width()
        height = surface_canvas.winfo_height()
        
        # Update text color
        surface_canvas.itemconfig("text", fill=hover_color)
        
        # Add animated highlight line
        highlight_line = surface_canvas.create_line(
        ...
        def animate_highlight(offset=0):
            if not button_state["active"]:
                return
                
            # Move line up and down for scanning effect
            new_y = height//2 + int(math.sin(offset) * (height//4))
            surface_canvas.coords(highlight_line, 0, new_y, width, new_y)
            
            # Schedule next animation frame
            surface_canvas.after(50, lambda: animate_highlight(offset + 0.3))
    ...
    def on_leave(e):
        ...
        # Reset appearance
        glow_frame.config(bg=shadow_color)
        inner_frame.config(highlightthickness=0)
        
        # Reset surface
        surface_canvas.delete("highlight")
        surface_canvas.itemconfig("text", fill=neon_color)
    
    def on_press(e):
        button_state["pressed"] = True
        
        # Create pressed effect
        bevel_frame.config(relief="sunken")
        mid_frame.config(bg=shadow_color)
        
        # Move text slightly for press effect
        surface_canvas.move("text", 1, 1)
    ...
    def on_release(e):
        ...
        # Reset from pressed state
        bevel_frame.config(relief="raised")
        mid_frame.config(bg=base_bg)
        
        # Move text back
        surface_canvas.move("text", -1, -1)
        
        # Call the command
        if button_state["active"]:  # Only if still hovering
            command()
    
    # Bind events
    surface_canvas.bind("<Enter>", on_enter)
    surface_canvas.bind("<Leave>", on_leave)
    surface_canvas.bind("<Button-1>", on_press)
    surface_canvas.bind("<ButtonRelease-1>", on_release)
    
    # Set fixed width for consistent button appearance
    button_width = len(text) * 10 + 60  # Adjust based on text length
    surface_canvas.config(width=button_width)
    
    # Make button activatable via keyboard for accessibility
    surface_canvas.config(takefocus=1)
    
    # Handle spacebar/enter activation
    def on_key(e):
        if e.keysym in ("Return", "space"):
            on_press(e)
            surface_canvas.after(100, lambda: on_release(e))
    
    surface_canvas.bind("<Key>", on_key)
    
    return shadow_frame

def setup_jarvis_ui(root_window, pause_func, resume_func):
    """Setup the JARVIS UI components with enhanced 3D futuristic design"""
    global root, status_label, animation_canvas, progress_bar, notification_label, notification_frame
    
    root = root_window
    root.title("J.A.R.V.I.S. Audio Interface")
    root.geometry("650x550")  # Slightly larger for enhanced effects
    root.attributes('-topmost', True)
    root.configure(bg=DEEP_SPACE_BG)
    ...
    try:
        try:
            ...
            print("Warning: eye.ico not found. Using default icon.")
            pass
    except Exception as e:
        print(f"Icon loading error: {e}")

    # Create main container with holographic border effect
    main_outer_frame = tk.Frame(root, bg=NEON_BLUE, bd=0)
    main_outer_frame.pack(fill="both", expand=True, padx=3, pady=3)
    
    # First layer of depth - dark shadow
    main_shadow_frame = tk.Frame(main_outer_frame, bg=METAL_SHADOW, bd=0)
    main_shadow_frame.pack(fill="both", expand=True, padx=1, pady=1)
    
    # Second layer - metallic highlight
    main_highlight_frame = tk.Frame(main_shadow_frame, bg=METAL_HIGHLIGHT, bd=1, relief="raised")
    main_highlight_frame.pack(fill="both", expand=True, padx=1, pady=1)
    
    # Main content area with deep space background
    main_frame = tk.Frame(main_highlight_frame, bg=DEEP_SPACE_BG, bd=0)
    main_frame.pack(fill="both", expand=True, padx=20, pady=20)
    
    # Title with advanced neon glow effect
    title_frame = tk.Frame(main_frame, bg=DEEP_SPACE_BG)
    title_frame.pack(pady=(5, 25))
    
    # Multi-layered title for depth illusion
    for offset in range(3, 0, -1):
        shadow_intensity = 70 + (3 - offset) * 30
        shadow_color = f"#{shadow_intensity:02x}{shadow_intensity:02x}{shadow_intensity+30:02x}"
        
        title_shadow = tk.Label(
            title_frame,
            text="J.A.R.V.I.S. AUDIO INTERFACE",
            font=(FONT_FAMILY, 20, "bold"),
            bg=DEEP_SPACE_BG,
            fg=shadow_color
        )
        title_shadow.place(x=offset, y=offset)

    # Main title with neon glow on top
    ...
    # Create hexagonal animation frame with metallic border and neon accents
    animation_container = tk.Frame(main_frame, bg=DEEP_SPACE_BG)
    animation_container.pack(fill="both", expand=True, pady=10)
    
    # Layered metallic frames for 3D depth effect
    animation_outer_shadow = tk.Frame(
        animation_container, 
        bg=METAL_SHADOW,
        bd=0
    )
    animation_outer_shadow.pack(fill="both", expand=True)
    
    # Outer frame with embossed edge effect
    animation_outer_frame = tk.Frame(
        animation_outer_shadow, 
        bg=METAL_HIGHLIGHT,
        bd=2, 
        relief="raised"
    )
    animation_outer_frame.pack(fill="both", expand=True, padx=2, pady=2)

    # Middle frame with inset effect and metallic texture
    animation_middle_frame = tk.Frame(
        animation_outer_frame, 
        bg=METAL_BG,
        bd=2, 
        relief="sunken"
    )
    animation_middle_frame.pack(fill="both", expand=True, padx=2, pady=2)
    
    # Inner frame with neon accent border
    animation_inner_frame = tk.Frame(
        animation_middle_frame, 
        bg=DEEP_SPACE_BG,
        bd=0,
        highlightbackground=NEON_BLUE,
        highlightcolor=NEON_CYAN,  # Dual-color effect
        highlightthickness=1
    )
    animation_inner_frame.pack(fill="both", expand=True, padx=5, pady=5)

    # Canvas with dark background for animation
    animation_canvas = tk.Canvas(
        animation_inner_frame,
        bg="#000000",
        bd=0,
        highlightthickness=0
    )
    ...
    # Status panel with recessed 3D design
    status_outer_frame = tk.Frame(
        main_frame, 
        bg=NEON_BLUE,  # Neon accent edge
        bd=0 
    )
    status_outer_frame.pack(fill="x", pady=(15, 5))

    status_shadow_frame = tk.Frame(
        status_outer_frame, 
        bg=METAL_SHADOW,  # Shadow edge
        bd=0
    )
    status_shadow_frame.pack(fill="x", padx=1, pady=1)
    
    status_bevel_frame = tk.Frame(
        status_shadow_frame, 
        bg=METAL_HIGHLIGHT,  # Highlight bevel
        bd=1, 
        relief="ridge"  # Ridge effect for more technologically advanced look
    )
    status_bevel_frame.pack(fill="x", padx=1, pady=1)

    status_frame = tk.Frame(
        status_bevel_frame, 
        bg=METAL_BG,  # Main metallic panel
        bd=2, 
        relief="sunken"  # Recessed panel effect
    )
    status_frame.pack(fill="x", padx=1, pady=1, ipady=5)

    # Status text with neon glow effect
    status_label = tk.Label(
        status_frame,
        text="● SYSTEM INITIALIZING ●",  # Added decorative elements
    ...
    # Enhanced 3D progress bar with neon glow and metallic trough
    progress_style = ttk.Style()
    try:
        available_themes = progress_style.theme_names()
        if 'alt' in available_themes:
            progress_style.theme_use('alt')
        elif 'clam' in available_themes:
            progress_style.theme_use('clam')
        else:
            progress_style.theme_use(available_themes[0])
    except tk.TclError:
        print("Warning: Could not set ttk theme.")

    # Create advanced 3D progress bar style
    progress_style_name = "JarvisNeon.Horizontal.TProgressbar"
    try:
        progress_style.layout(progress_style_name, 
        ...
        progress_style.configure(
            progress_style_name,
            thickness=15,  # Thicker for more presence
            background=NEON_ORANGE,  # Neon fill
            troughcolor=METAL_BG,
            bordercolor=METAL_SHADOW,
            lightcolor=NEON_BLUE,  # Neon edge highlight
            darkcolor=METAL_SHADOW,
            relief="flat"
        )
    except tk.TclError as e:
        print(f"Warning: Could not configure progress bar style: {e}")
        progress_style_name = "Horizontal.TProgressbar"  # Fallback

    # Progress bar container with 3D border
    progress_container = tk.Frame(main_frame, bg=METAL_SHADOW, bd=0)
    progress_container.pack(fill="x", pady=(10, 20))
    
    progress_highlight = tk.Frame(progress_container, bg=METAL_HIGHLIGHT, bd=1, relief="raised")
    progress_highlight.pack(fill="x", padx=1, pady=1)
    ...
    progress_bar.pack(fill="x", padx=2, pady=2, ipady=2)

    # Control panel with metallic finish
    control_panel_outer = tk.Frame(main_frame, bg=METAL_SHADOW, bd=0)
    control_panel_outer.pack(fill="x", pady=10)
    
    control_panel_highlight = tk.Frame(control_panel_outer, bg=METAL_HIGHLIGHT, bd=1, relief="raised")
    control_panel_highlight.pack(fill="x", padx=1, pady=1)
    
    control_panel = tk.Frame(control_panel_highlight, bg=METAL_BG, bd=1, relief="sunken")
    control_panel.pack(fill="x", padx=1, pady=1)
    
    # Buttons frame with centered alignment
    buttons_frame = tk.Frame(control_panel, bg=METAL_BG)
    buttons_frame.pack(pady=15)

    # Start/Resume button (Primary)
    start_button_frame = create_button(
        buttons_frame,
        "◉ ACTIVATE SYSTEM",
        resume_func,
        True
    )
    start_button_frame.pack(side="left", padx=25)

    # Stop button (Secondary)
    stop_button_frame = create_button(
        buttons_frame,
        "◉ DEACTIVATE SYSTEM",
        pause_func,
        False
    )
    stop_button_frame.pack(side="left", padx=25)

    # Initialize notification components to None
    notification_frame = None
    notification_label = None

    # Start animation loop with initial state
    update_animation("INITIALIZING...")
    root.after(500, lambda: update_status("Waiting for sound...")) # Transition after init

    # Key bindings
    root.bind('<space>', lambda e: resume_func())
    root.bind('<Escape>', lambda e: pause_func())
    
    # Bind keys to buttons for accessibility focus indication
    # Focus the shadow_frame which contains the button visually
    root.bind('<Prior>', lambda e: start_button_frame.focus_set()) # PageUp for Start
    root.bind('<Next>', lambda e: stop_button_frame.focus_set()) # PageDown for Stop
...
if __name__ == "__main__":
    ...
    def dummy_pause():
        ...
        root_window.after(100, lambda: show_notification("Recording Deactivated"))


    def dummy_resume():
        print("Resume requested")
        update_status("Recording in progress...") # Simulate recording on resume
        root_window.after(100, lambda: show_notification("Recorder Activated"))
    ...
    # root_window.after(15100, lambda: show_notification("Processing Complete"))
    # root_window.after(18000, lambda: update_status("Waiting for sound..."))
