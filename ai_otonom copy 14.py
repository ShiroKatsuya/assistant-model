...
def internet_akses():
    ...
    def run_main_loop(translate):
        search_results = websearch(translate)
        ...
        # Debug: Print the initial messages before starting the iteration loop.
        print(f"{Colors.OKCYAN}Initial messages: {json.dumps(messages, indent=2)}{Colors.ENDC}")
        ...
        while iteration < max_iterations:
            ...
            # Print the messages being sent this iteration.
            print(f"{Colors.OKBLUE}Messages before completion request:\n{json.dumps(messages, indent=2)}{Colors.ENDC}")
            ...
            try:
                ...
                # Debug: Print raw response from the completion API.
                print(f"{Colors.OKBLUE}Raw response from completion:\n{response}{Colors.ENDC}")
                ...
                print(f"{Colors.OKBLUE}Assistant content received: {assistant_content}{Colors.ENDC}")
                ...
                if tool_calls and isinstance(tool_calls, list):
                    for tool_call in tool_calls:
                        ...
                        print(f"{Colors.OKBLUE}Result from {function_name}: {serialized_tool_result}{Colors.ENDC}")
                        messages.append({
                            'role': 'tool',
                            'name': function_name,
                            'content': serialized_tool_result
                        })
                    # Check if any tool call marked task completion.
            ...
            print(f"{Colors.OKCYAN}=== End of Iteration {iteration+1} ==={Colors.ENDC}\n")
            iteration += 1
            sleep(2)
    ...
    return run_main_loop, show_learned_data


if __name__ == "__main__":
    run_main_loop, show_learned_data = internet_akses()  # Unpack both functions

    audio_processor = process_audio()
    resume_audio_processing()
    audio_count = 0  # Counter to track the number of audio inputs received
    try:
        while True:
            try:
                ...
                if translate:
                    audio_count += 1
                    # Only process the input starting from the second audio message
                    if audio_count < 2:
                        print("Received first audio input, awaiting second input to begin processing...")
                        resume_audio_processing()
                        continue
                    pause_audio_processing()
                    run_main_loop(translate)
                    resume_audio_processing()
                    if any(keyword in translate.lower() for keyword in ["stop internet", "hentikan internet", "matikan internet"]):
