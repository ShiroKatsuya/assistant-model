...
def process_audio(text):
    ...
    try:
        ...
        if audio_buffer:            
            ...
            # Wait to ensure visualization starts
            timeout = 15  # Increased timeout for visualization to start
            start_time = time.time()
            while playback_start_time is None and time.time() - start_time < timeout:
                time.sleep(0.1)
                
            if playback_start_time is None:
                print("Warning: Visualization did not start after audio generation")
                return False
    ...
    try:
        ...
        if playback_start_time and audio_duration:
            ...
            if current_time - playback_start_time > audio_duration and pygame.mixer.get_init() and pygame.mixer.music.get_busy() == 0:
                ...
                print("Audio playback complete, cleaning up threads while keeping visualization")
...
def start_visualization(text):
    ...
    if audio_thread and audio_thread.is_alive():
        print("Cancelling existing audio thread")
...
def run_with_inputs(input_text):
    ...
    try:
        if visualization_running:
            ...
            audio_started = False
            print("\nInput processed. Waiting for audio playback to start...")
            
            # Wait for audio to start (playback_start_time will be set)
            timeout_start = time.time()
            max_start_wait = 30  # Increased timeout for audio generation
            ...
            if not audio_started:
                print("Warning: Audio playback didn't start within timeout period. Visualization may have failed.")
                
                # Check if audio thread had errors
                if audio_thread and audio_thread.is_alive():
                    print("Audio thread is still running but playback didn't start - this may indicate an issue with audio generation")
                    # Join the thread with a timeout in case it's stuck
                    audio_thread.join(timeout=5)  # Increased timeout for joining
                
                visualization_complete_event.set()  # Signal completion even in case of error
                return False
                
            # Now wait for audio playback to complete
            print("Audio started. Waiting for playback to complete...")
            
            # Wait for the audio to finish playing
            max_playback_wait = 120  # Maximum 2 minutes to wait for audio playback
            ...
            if pygame.mixer.get_init() and pygame.mixer.music.get_busy():
                print("Warning: Audio playback exceeded maximum wait time")
                pygame.mixer.music.stop()
            ...
            if audio_thread and audio_thread.is_alive():
                ...
                audio_thread.join(timeout=10)  # Increased timeout for thread completion
                
            # Process any remaining events to keep the visualization responsive
            for _ in range(10):  # Process a few event cycles
                plt.pause(0.1)
                if plt.fignum_exists(plt.gcf().number):
                    plt.gcf().canvas.flush_events()
                
            # Signal that all threads are complete
            visualization_complete_event.set()
            return True
        ...
        # Just return to allow the next input to be processed
        print("Audio completed, ready for next input.")
        # Ensure the event is set even if we missed setting it elsewhere
