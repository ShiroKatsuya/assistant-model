            ...
            if any(keyword in translate.lower() for keyword in ["create image", "create images", "buatkan saya gambar", "gambar", "image", "buatkan gambar", "picture", "pictures", "photo", "photos", "Draw", "draw", "Make"]):
                ...
                try:
                    ...
                    with visualization_lock:
                        ...
                        # Clean up response
                        cleaned_response = response.text.replace('*', '').replace('\n\n', '\n')
                        print(f"Generated response: {cleaned_response[:50]}...")
                        ...
                        try:
                            ...
                            while not visualization_completed and retry_count < max_retries:
                                ...
                                if not visualization_completed:
                                    print(f"Visualization attempt {retry_count + 1} failed, {'retrying' if retry_count < max_retries - 1 else 'giving up'}")
                                    retry_count += 1
                                    time.sleep(2)  # Longer pause before retrying
                                    # Reset the event for the next attempt
                                    visualization_complete_event.clear()
                            
                            # Wait for the visualization to fully complete using the event
                            print("Waiting for visualization threads to complete...")
                            # Timeout of 45 seconds to wait for visualization to complete
                            if not visualization_complete_event.wait(timeout=45):
                                print("Warning: Timed out waiting for visualization completion event")
                            else:
                                print("Visualization complete event received")
                            
                            # Add a delay to ensure all visualization threads have truly finished
                            time.sleep(2)
                            ...
                            if visualization_completed:
                                ...
                                print("Visualization failed after multiple attempts")
                        finally:
