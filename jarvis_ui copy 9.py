...
def create_button(parent, text, command, is_primary=True):
    """Create a futuristic 3D-style button"""
    
    neon_color = NEON_ORANGE if is_primary else NEON_BLUE
    
    # Outer frame for 3D depth and metallic look
    outer_frame = tk.Frame(
        parent, 
        bg=METAL_HIGHLIGHT, # Lighter edge for highlight
        bd=1, 
        relief="raised" # Outer raised edge
    )
    
    # Middle frame for main button body/color
    button_frame = tk.Frame(
        outer_frame, 
        bg=METAL_BG, # Darker metallic background
        bd=1, 
        relief="sunken" # Inner sunken edge
    )
    button_frame.pack(padx=1, pady=1) # Padding inside the outer frame

    # Inner button - make it look flat but glowing
    button = tk.Button(
        button_frame,
        text=text,
        font=(FONT_FAMILY, 10, "bold"),
        bg=METAL_BG, # Match button frame background
        fg=neon_color, # Neon text color
        activebackground=METAL_BG, # Keep bg same when clicked
        activeforeground="#FFFFFF", # Bright white text when clicked
        relief="flat", # Keep the text area flat
        bd=0, # No border for the button itself
        highlightthickness=0, # No focus highlight
        command=command,
        padx=20, # More horizontal padding
        pady=8   # More vertical padding
    )
    button.pack(padx=5, pady=5) # Padding inside the button frame
    
    # --- Simulate dynamic light effect on hover ---
    # Change background slightly and text glow intensifies
    def on_enter(e):
        button.config(bg=METAL_HIGHLIGHT, fg="#FFFFFF") # Lighter bg, white text
        outer_frame.config(bg=neon_color) # Outer frame glows
        button_frame.config(relief="raised", bg=METAL_HIGHLIGHT) # Inner frame pops out slightly

    def on_leave(e):
        button.config(bg=METAL_BG, fg=neon_color) # Back to normal
        outer_frame.config(bg=METAL_HIGHLIGHT) # Back to normal
        button_frame.config(relief="sunken", bg=METAL_BG) # Back to normal
    ...
    return outer_frame # Return the outermost frame

def setup_jarvis_ui(root_window, pause_func, resume_func):
    """Setup the JARVIS UI components with a futuristic design"""
    global root, status_label, animation_canvas, progress_bar, notification_label
    
    root = root_window
    root.title("J.A.R.V.I.S. Audio Interface") # Updated title
    root.geometry("550x450") # Slightly larger window
    root.attributes('-topmost', True)
    root.configure(bg=DEEP_SPACE_BG) # Use the deep space background
    ...
    main_frame = tk.Frame(root, bg=DEEP_SPACE_BG, bd=0)
    ...
    title_label = tk.Label(
        ...
        bg=DEEP_SPACE_BG,
        fg=NEON_BLUE # Use neon blue for title
    )
    title_label.pack(pady=(0, 25))
    
    # Animation Area - Frame with metallic border
    animation_outer_frame = tk.Frame(
        main_frame, 
        bg=METAL_HIGHLIGHT, # Highlight border color
        bd=1, 
        relief="groove" # Groove for metallic feel
    )
    animation_outer_frame.pack(fill="both", expand=True, pady=15)

    animation_frame = tk.Frame(
        animation_outer_frame, 
        bg=METAL_BG, # Inner metallic panel
        bd=1, 
        relief="sunken" 
    )
    animation_frame.pack(fill="both", expand=True, padx=2, pady=2)

    animation_canvas = tk.Canvas(
        animation_frame,
        bg="#000000", # Keep canvas black
        bd=0,
        highlightthickness=1,
        highlightbackground=NEON_BLUE, # Neon blue highlight border for canvas
        height=150 
    )
    animation_canvas.pack(fill="both", expand=True, padx=5, pady=5)

    # Status Frame - Recessed panel look
    status_frame = tk.Frame(
        main_frame, 
        bg=METAL_BG, # Use metallic bg
        bd=2, 
        relief="sunken" # Sunken effect
    )
    status_frame.pack(fill="x", pady=(15, 5), ipady=5) # Internal padding

    status_label = tk.Label(
        status_frame,
        text="Awaiting Input...", # Updated initial text
        font=(FONT_FAMILY, 12, "italic"),
        bg=METAL_BG, # Match frame background
        fg=NEON_BLUE # Neon blue status text
    )
    ...
    # Create the layout for the custom progressbar
    progress_style.layout("Horizontal.Neon.TProgressbar", 
    ...
    progress_style.configure(
        "Neon.TProgressbar", # Custom style name
        thickness=10, # Thicker bar
        background=NEON_ORANGE, # Neon orange fill
        troughcolor=METAL_BG, # Metallic trough
        borderwidth=1,
        relief="flat" # Flat bar
    )

    progress_bar = ttk.Progressbar(
        main_frame,
        style="Neon.TProgressbar", # Use the custom style
        orient="horizontal",
        # length=460, # Let it fill 'x'
        mode="determinate",
        value=0 
    )
    progress_bar.pack(fill="x", pady=(10, 20), ipady=2) # Add some internal padding

    # Buttons frame - Centered
    buttons_frame = tk.Frame(main_frame, bg=DEEP_SPACE_BG)
    buttons_frame.pack(pady=15)

    # Start/Resume button (Primary)
    start_button = create_button(
        buttons_frame,
        "ACTIVATE RECORDER", # Changed text
        resume_func,
        True # is_primary = True (Neon Orange)
    )
    ...
    stop_button = create_button(
        buttons_frame,
        "DEACTIVATE RECORDER", # Changed text
        pause_func,
        False # is_primary = False (Neon Blue)
    )
    stop_button.pack(side="left", padx=15)

    # Notification label (hidden initially) - Futuristic style
    notification_label = tk.Label(
        root, 
        text="Notification Placeholder",
        font=(FONT_FAMILY, 10, "bold"),
        bg=NEON_BLUE, # Neon background
        fg=DEEP_SPACE_BG, # Dark text
        padx=15,
        pady=8,
        relief="raised", 
        bd=1
    )
    # Placement handled by show_notification

    # Start animation loop
    update_animation("Awaiting Input...")

    # Key bindings
    root.bind('<space>', lambda e: resume_func())
    root.bind('<Escape>', lambda e: pause_func())
    
    # Bind keys to buttons for accessibility focus indication (optional visual cue)
    root.bind('<Prior>', lambda e: start_button.focus_set()) # PageUp for Start
    root.bind('<Next>', lambda e: stop_button.focus_set()) # PageDown for Stop
