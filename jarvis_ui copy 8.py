...
def show_notification(message):
    """Show a temporary notification with futuristic design"""
    global notification_label, root
    
    if notification_label:
        # Configure the notification with enhanced futuristic styling
        notification_label.config(
            text=message,
            font=(FONT_FAMILY, 10, "bold"),
            bg=DEEP_SPACE_BG,  # Dark background
            fg=NEON_BLUE,      # Neon text
            padx=20,
            pady=10,
            relief="flat", 
            bd=0
        ...
        # Create a frame border effect for the notification
        def update_border_color(count=0, direction=1):
            colors = [NEON_BLUE, "#50c2ff", "#90d5ff", "#50c2ff", NEON_BLUE]
            if count < len(colors):
                notification_label.config(highlightbackground=colors[count], highlightthickness=2)
                notification_label.after(150, lambda: update_border_color(count+direction, direction))
            else:
                # Keep the border pulsing while notification is visible
                notification_label.after(150, lambda: update_border_color(count-1, -1) if count > 0 else update_border_color(1, 1))
                
        # Position and show with fade-in effect
        notification_label.place(relx=0.5, y=15, anchor="n")
        notification_label.config(fg=DEEP_SPACE_BG)  # Start with invisible text
        
        # Simulate fade-in
        def fade_in(alpha=0):
            if alpha <= 10:
                # Gradually change text color from bg to neon
                r = int((1-alpha/10) * int(DEEP_SPACE_BG[1:3], 16) + (alpha/10) * int(NEON_BLUE[1:3], 16))
                g = int((1-alpha/10) * int(DEEP_SPACE_BG[3:5], 16) + (alpha/10) * int(NEON_BLUE[3:5], 16))
                b = int((1-alpha/10) * int(DEEP_SPACE_BG[5:7], 16) + (alpha/10) * int(NEON_BLUE[5:7], 16))
                color = f"#{r:02x}{g:02x}{b:02x}"
                notification_label.config(fg=color)
                notification_label.after(50, lambda: fade_in(alpha+1))
            else:
                # Start the border animation once fade-in is complete
                update_border_color()
                # Schedule fade-out and removal
                notification_label.after(2500, fade_out)
        
        # Simulate fade-out
        def fade_out(alpha=10):
            if alpha >= 0:
                # Gradually change text color from neon to bg
                r = int((1-alpha/10) * int(DEEP_SPACE_BG[1:3], 16) + (alpha/10) * int(NEON_BLUE[1:3], 16))
                g = int((1-alpha/10) * int(DEEP_SPACE_BG[3:5], 16) + (alpha/10) * int(NEON_BLUE[3:5], 16))
                b = int((1-alpha/10) * int(DEEP_SPACE_BG[5:7], 16) + (alpha/10) * int(NEON_BLUE[5:7], 16))
                color = f"#{r:02x}{g:02x}{b:02x}"
                notification_label.config(fg=color)
                notification_label.after(50, lambda: fade_out(alpha-1))
            else:
...
def create_button(parent, text, command, is_primary=True):
    """Create a futuristic 3D-style button with advanced lighting effects"""
    
    neon_color = NEON_ORANGE if is_primary else NEON_BLUE
    neon_dim = "#FF7300" if is_primary else "#0070FF"  # Slightly dimmer version
    neon_bright = "#FFA500" if is_primary else "#00BFFF"  # Brighter version
    
    # Create multiple frames for enhanced 3D depth effect
    # Outermost frame - metallic border with chamfered edge effect
    outer_frame = tk.Frame(
        parent, 
        bg=METAL_HIGHLIGHT, 
        bd=2, 
        relief="ridge",  # Ridge gives a beveled edge appearance
        highlightbackground=neon_dim,  # Subtle neon outline
        highlightthickness=1
    )
    
    # Middle frame - creates depth and metallic sheen
    middle_frame = tk.Frame(
        outer_frame, 
        bg="#1E1E1E",  # Darker than METAL_BG for depth
        bd=1, 
        relief="groove"  # Groove adds texture
    )
    middle_frame.pack(padx=1, pady=1)
    
    # Inner button frame - recessed area
    button_frame = tk.Frame(
        middle_frame, 
        bg=METAL_BG,
        bd=1, 
        relief="sunken"  # Sunken adds depth
    )
    button_frame.pack(padx=1, pady=1)

    # Actual button with glowing text effect
    button = tk.Button(
        button_frame,
        text=text,
        font=(FONT_FAMILY, 10, "bold"),
        bg=METAL_BG,
        fg=neon_color,  # Neon text
        activebackground="#2A2A2A",  # Slightly lighter when clicked
        activeforeground="#FFFFFF",  # White text when clicked
        relief="flat",  # Keep text area flat
        bd=0,  # No border for button itself
    ...
    # Glowing border effect variables
    glow_active = False
    glow_job = None
    
    # Enhanced hover effects with pulsing glow
    def on_enter(e):
        nonlocal glow_active, glow_job
        button.config(bg="#2A2A2A", fg="#FFFFFF")  # Lighter bg, white text
        
        # Cancel any existing glow animation
        if glow_job:
            button.after_cancel(glow_job)
            
        # Start the pulsing glow effect
        glow_active = True
        pulse_glow(0, 1)
    
    def pulse_glow(step=0, direction=1):
        nonlocal glow_job
        if not glow_active:
            return
            
        # Create a pulsing effect with multiple color frames
        if direction > 0:
            # Brightening phase
            if step < 5:
                # Gradually change color from dim to bright
                outer_frame.config(highlightbackground=neon_bright, highlightthickness=step//2 + 1)
                glow_job = outer_frame.after(100, lambda: pulse_glow(step+1, direction))
            else:
                # Switch direction
                glow_job = outer_frame.after(100, lambda: pulse_glow(step-1, -1))
        else:
            # Dimming phase
            if step > 0:
                # Gradually change color from bright to dim
                outer_frame.config(highlightbackground=neon_dim, highlightthickness=step//2 + 1)
                glow_job = outer_frame.after(100, lambda: pulse_glow(step-1, direction))
            else:
                # Switch direction
                glow_job = outer_frame.after(100, lambda: pulse_glow(step+1, 1))
    ...
    def on_leave(e):
        nonlocal glow_active, glow_job
        button.config(bg=METAL_BG, fg=neon_color)  # Back to normal
        
        # Stop the pulsing glow effect
        glow_active = False
        if glow_job:
            button.after_cancel(glow_job)
            
        # Reset the border
        outer_frame.config(highlightbackground=neon_dim, highlightthickness=1)
    
    # Button pressed effect - more dramatic than hover
    def on_press(e):
        # Quick flash of brightness when pressed
        button.config(bg="#3A3A3A", fg="#FFFFFF")
        outer_frame.config(highlightbackground="#FFFFFF", highlightthickness=2)
        # Schedule return to hover state
        button.after(150, lambda: button.config(bg="#2A2A2A", fg="#FFFFFF") if glow_active else button.config(bg=METAL_BG, fg=neon_color))
        outer_frame.after(150, lambda: outer_frame.config(highlightbackground=neon_bright if glow_active else neon_dim, 
                                                       highlightthickness=2 if glow_active else 1))
        
    # Bind hover and press events
    button.bind("<Enter>", on_enter)
    button.bind("<Leave>", on_leave)
    button.bind("<Button-1>", on_press)  # Left mouse button press
    
    # Custom key focus visuals
    def on_focus_in(e):
        on_enter(e)  # Use same effects as hover
    
    def on_focus_out(e):
        on_leave(e)  # Use same effects as leaving hover
...
def setup_jarvis_ui(root_window, pause_func, resume_func):
    """Setup the JARVIS UI components with an advanced futuristic holographic design"""
    global root, status_label, animation_canvas, progress_bar, notification_label
    
    root = root_window
    root.title("J.A.R.V.I.S. Neural Interface")  # More futuristic title
    root.geometry("600x500")  # Slightly larger window for more details
    ...
    # Create a main container with holographic border effect
    main_container = tk.Frame(root, bg=DEEP_SPACE_BG, bd=0)
    main_container.pack(fill="both", expand=True, padx=5, pady=5)
    
    # Add glowing border to main container
    ...
    def pulse_border():
        ...
        for i, color in enumerate(colors):
            border_frame.after(i*150, lambda c=color: border_frame.config(highlightbackground=c))
        border_frame.after(len(colors)*150, pulse_border)
    
    # Start border pulsing
    pulse_border()
    
    # Main frame with padding and metallic texture
    main_frame = tk.Frame(border_frame, bg=DEEP_SPACE_BG, bd=0)
    main_frame.pack(fill="both", expand=True, padx=20, pady=20)
    
    # Create header with futuristic style
    header_frame = tk.Frame(main_frame, bg=METAL_BG, bd=0, height=40)
    header_frame.pack(fill="x", pady=(0, 15))
    
    # Add decorative elements to header
    for i in range(3):
        indicator = tk.Canvas(header_frame, width=15, height=15, bg=METAL_BG, 
                             highlightthickness=0)
        indicator.pack(side="right", padx=5, pady=5)
        indicator.create_oval(2, 2, 13, 13, fill=NEON_ORANGE if i == 0 else NEON_BLUE, 
                             outline=METAL_HIGHLIGHT)
    
    # Title with futuristic font and glow effect
    title_frame = tk.Frame(main_frame, bg=DEEP_SPACE_BG)
    title_frame.pack(pady=(0, 20))
    
    title_label = tk.Label(
        title_frame,
        text="J.A.R.V.I.S. NEURAL INTERFACE",
    ...
    subtitle = tk.Label(
        title_frame,
        text="QUANTUM AUDIO PROCESSING SYSTEM v3.7",
        font=(FONT_FAMILY, 8),
        bg=DEEP_SPACE_BG,
        fg="#80C0FF",  # Lighter blue
        bd=0
    )
    ...
    def title_glow():
        colors = [NEON_BLUE, "#50c2ff", "#90d5ff", "#50c2ff", NEON_BLUE]
        for i, color in enumerate(colors):
            title_label.after(i*200, lambda c=color: title_label.config(fg=c))
        title_label.after(len(colors)*200, title_glow)
    
    # Start title glow
    title_glow()
    
    # Animation Area - Enhanced with futuristic border
    ...
    animation_outer_frame.pack(fill="both", expand=True, pady=10)
    
    # Add decorative corner elements
    for corner in ["nw", "ne", "sw", "se"]:
        corner_canvas = tk.Canvas(animation_outer_frame, width=15, height=15, 
                                 bg=METAL_HIGHLIGHT, highlightthickness=0)
        if corner == "nw":
            corner_canvas.place(x=0, y=0)
        elif corner == "ne":
            corner_canvas.place(x=animation_outer_frame.winfo_reqwidth()-15, y=0)
        elif corner == "sw":
            corner_canvas.place(x=0, y=animation_outer_frame.winfo_reqheight()-15)
        else:
            corner_canvas.place(x=animation_outer_frame.winfo_reqwidth()-15, 
                               y=animation_outer_frame.winfo_reqheight()-15)
        
        # Draw corner design
        if corner == "nw":
            corner_canvas.create_line(0, 5, 5, 0, fill=NEON_BLUE, width=2)
        elif corner == "ne":
            corner_canvas.create_line(15, 5, 10, 0, fill=NEON_BLUE, width=2)
        elif corner == "sw":
            corner_canvas.create_line(0, 10, 5, 15, fill=NEON_BLUE, width=2)
        else:
            corner_canvas.create_line(15, 10, 10, 15, fill=NEON_BLUE, width=2)
    ...
    animation_frame.pack(fill="both", expand=True, padx=3, pady=3)
    ...
    # Add decorative grid lines to the canvas
    for i in range(0, 200, 20):
        animation_canvas.create_line(0, i, 2000, i, fill="#0A1A2A", width=1)
        animation_canvas.create_line(i, 0, i, 2000, fill="#0A1A2A", width=1)
    ...
    # Animate indicator
    def pulse_indicator():
        colors = [NEON_ORANGE, "#FFA04D", "#FFBF80", "#FFA04D", NEON_ORANGE]
        for i, color in enumerate(colors):
            indicator_canvas.after(i*150, lambda c=color: indicator_canvas.config(bg=c))
        indicator_canvas.after(len(colors)*150, pulse_indicator)
    
    # Start indicator pulsing
    pulse_indicator()
    ...
    status_label = tk.Label(
        status_frame,
        text="SYSTEM READY • AWAITING VOICE INPUT",
    ...
    # Add animated ellipsis to status
    def animate_ellipsis():
        current_text = status_label.cget("text")
        base_text = current_text.rstrip(".")
        ellipsis_count = current_text.count(".") % 3 + 1
        status_label.config(text=base_text + "."*ellipsis_count)
        status_label.after(500, animate_ellipsis)
    ...
    progress_style.layout("Neon.TProgressbar", 
    ...
    # Frame for progress bar with enhanced border
    ...
    # Animate progress bar when inactive
    def pulse_progress():
        if progress_bar['value'] == 0:  # Only pulse when not in active use
            progress_bar.config(value=5)
            progress_bar.after(500, lambda: progress_bar.config(value=0))
            progress_bar.after(1000, pulse_progress)
        else:
            progress_bar.after(1000, pulse_progress)
    
    pulse_progress()

    # Control panel with enhanced metallic frame
    ...
    # Add decorative elements to control panel
    for i in range(2):
        separator = tk.Frame(control_panel, width=2, height=50, bg=NEON_BLUE if i == 0 else NEON_ORANGE)
        separator.pack(side="left", padx=10, fill="y", pady=5)
    ...
    stop_button = create_button(
        buttons_frame,
        "DEACTIVATE NEURAL INTERFACE",
        pause_func,
        False
    )
    stop_button.pack(side="left", padx=10)
    
    # Add right decorative elements
    for i in range(2):
        separator = tk.Frame(control_panel, width=2, height=50, bg=NEON_ORANGE if i == 0 else NEON_BLUE)
        separator.pack(side="right", padx=10, fill="y", pady=5)

    # Create notification label with enhanced style but don't place it yet
    ...
    # Start animation loop for audio visualizer
    update_animation("SYSTEM READY")

    # Key bindings
    root.bind('<space>', lambda e: resume_func())
    root.bind('<Escape>', lambda e: pause_func())
    root.bind('<Prior>', lambda e: start_button.focus_set())
    root.bind('<Next>', lambda e: stop_button.focus_set())
