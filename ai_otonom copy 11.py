...
def learn_from_url(url):
    try:
        ...
        if isinstance(new_content, str):
            return f"Failed to fetch content from {url}: {new_content}"
        
        # Check if the URL already exists and compare the content.
        if url in kb:
            if kb[url] == new_content:
                print(f"{Colors.OKCYAN}Content from {url} is unchanged. Overwriting record with latest data.{Colors.ENDC}")
            else:
                print(f"{Colors.OKCYAN}Content from {url} has changed. Updating the record.{Colors.ENDC}")
        else:
            print(f"{Colors.OKCYAN}Saving new content from {url}.{Colors.ENDC}")
        
        # Overwrite (or create) the knowledge base entry for this URL.
        kb[url] = new_content
